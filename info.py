...
API_ID = int(environ.get('API_ID', '15787995'))
API_HASH = environ.get('API_HASH', 'e51a3154d2e0c45e5ed70251d68382de')
OWNER_ID = environ.get('OWNER_ID', '7046488481')
BOT_TOKEN = environ.get('BOT_TOKEN', "7710795424:AAHgJbnbJMgEMwvWm0I7SqKxgBnFX0gLD94")
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7046488481 5115691197').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002069323367').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '7046488481').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '6497757690').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002069323367') #Channel / Group Id for force sub ( make sure bot is admin )
...
# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://Kali:SHM14002022SHM@cluster0.bxsct.mongodb.net/myDatabase?retryWrites=true&w=majority")
...
#Auto approve 
CHAT_ID = [int(app_chat_id) if id_pattern.search(app_chat_id) else app_chat_id for app_chat_id in environ.get('CHAT_ID', '7046488481').split()]
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002410423843')) #Log channel id ( make sure bot is admin )
DUMP_CHNL = int(environ.get('DUMP_CHNL', '-1002069323367'))
